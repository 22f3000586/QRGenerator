...
export default function Home() {
  ...
  const [fgColor, setFgColor] = useState("#000000"); // Set default pattern to your purple
  ...
  const getDownloadUrl = (ext) => {
    if (!qrUrl) return "#";
    return `/api/download?image_url=${encodeURIComponent(qrUrl)}&format=${ext}`;
  };
...
<div className="flex gap-3 overflow-x-auto pb-4">
  ...
  className={`flex-shrink-0 w-14 h-14 rounded-2xl flex items-center justify-center text-lg bg-white hover:bg-slate-50 transition
    ${!logo ? "border-2 border-[#6d55a2]" : "border-2 border-dashed border-slate-200"}
  `}
  title="Remove logo"
>
  ✕
...
{[
  ...
  <button
    ...
    onClick={async () => {
      ...
      const file = new File([blob], src.split("/").pop() || "logo.png", {
        type: blob.type || "image/png",
      });

      setLogo(file);
      setSelectedLogoSrc(src); // ✅ track selected logo
    }}
    ...
    title="Use this logo"
  >
    <img src={src} alt="logo" className="w-9 h-9 object-contain" />
  </button>
))}
                    ...
                    <div className="mt-4">
                      <label className="block text-xs font-bold text-slate-400 mb-2 uppercase">Custom</label>
                      <input
  type="file"
  accept="image/*"
  onChange={(e) => {
    ...
    setLogo(f);
    setSelectedLogoSrc(null); // because it's custom
  }}
...
/>
                ...
                {activeTab === "Color" && (
                  <div className="grid sm:grid-cols-2 gap-6">
                    <div>
                      <label className="block text-xs font-bold text-slate-400 mb-2 uppercase">Pattern Color</label>
                    ...
                    <div>
                      <label className="block text-xs font-bold text-slate-400 mb-2 uppercase">Background</label>
                      <div className="flex items-center gap-3 bg-white p-2 rounded-xl border border-slate-100">
                        <input
                          type="color"
                          value={bgColor}
              ...
              <div className="mt-8">
                ...
                <div className="flex gap-4 overflow-x-auto pb-2">
                  {history.slice(0, 5).map((item) => (
                    ...
                    >
                      <img src={item.image_url} alt="qr" className="w-full h-full rounded-xl object-contain" />
                    </div>
                  ))}
                </div>
              </div>
          ...
          <section className="lg:col-span-6 flex items-center justify-center">
            <div className="w-full max-w-[400px] aspect-square rounded-[48px] bg-gradient-to-br from-[#f8f7fb] to-white border border-white shadow-xl p-8 flex flex-col items-center justify-center relative">
              <div className="w-full aspect-square bg-white rounded-[32px] shadow-sm flex items-center justify-center p-6 mb-8">
                {qrUrl ? (
                  <img src={qrUrl} alt="QR Preview" className="w-full h-full object-contain animate-in fade-in zoom-in duration-300" />
                ) : (
              ...
              <div className="w-full relative">
                ...
                >
                  Download 
                  <span className={`transition-transform duration-200 ${downloadOpen ? 'rotate-180' : ''}`}>▾</span>
                </button>
                ...
                {downloadOpen && qrUrl && (
                  <div className="absolute bottom-full left-0 right-0 mb-3 rounded-2xl bg-white/90 backdrop-blur-md border border-white shadow-2xl overflow-hidden animate-in slide-in-from-bottom-4 duration-200">
                    {[{ label: "PNG Image", ext: "png" }, { label: "Vector SVG", ext: "svg" }, { label: "Print PDF", ext: "pdf" }].map((opt) => (
                      <a
                        key={opt.ext}
                        href={getDownloadUrl(opt.ext)}
                        className="block px-6 py-4 font-bold text-slate-700 hover:bg-[#f1eff7] hover:text-[#6d55a2] transition border-b border-slate-50 last:border-none"
